...
class CODA(pl.LightningModule):
    def __init__(self, batch_size=None, lr=None, num_workers=None, **kwargs):
        ...
        self.dec_tok.add_special_tokens({
            'additional_special_tokens': ['<eos>', '<pth>', '<ctx>', '<tgt>', '<res>']
        })
    ...
    def setup(self, stage: str = None):
        if stage == 'fit':
            train_data = []
            with open('data/triple_dialog_data/coda_conv_gen_data_train.json', 'r', encoding='utf-8') as f:
                for row in f:
                    data = json.loads(row)
                    train_data.append(data)
            self.train_dataset = train_data
            dev_data = []
            with open('data/triple_dialog_data/coda_conv_gen_data_dev.json', 'r', encoding='utf-8') as f:
                for row in f:
                    data = json.loads(row)
            ...
            test_data = []
            with open('2_dialog_generation/construct_test_data.py', 'r', encoding='utf-8') as f:
                for row in f:
                    data = json.loads(row)
    ...
    def collate_fn(self, batch):
        ...
        for item in batch:
            ...
            input_seq = '<pth>' + path + '<ctx>' + context[0] + '<tgt>' + context[-1]
...
if __name__ == '__main__':
    ...
    tb_logger = pl_loggers.TensorBoardLogger('3_baselines/logs_coda', name='')
...
# nohup python -u 3_baselines/train_CODA.py > 3_baselines/logs/train_CODA_12.7.log 2>&1
