        ...
        for batch_data in tqdm([test_dataset[i:i+64] for i in range(0, len(test_dataset), 64)]):
            ...
            for path, context, target in zip(batch['path'], batch['context'], batch['target']):
                ...
                while True:
                    ...
                    user_predict_inputs.append(ctx_ids + [user_model.RES]) 
                    user_predict_mask.append([1] * len(user_predict_inputs[-1]))
                    pad_to_max_seq_len(user_predict_inputs, pad_token_id=user_model.EOS, max_seq_len=32)
                    pad_to_max_seq_len(user_predict_mask, pad_token_id=0, max_seq_len=32)
                    ids = torch.tensor(user_predict_mask).to('cuda')
                    mask = torch.tensor(user_predict_mask).to('cuda')
                    ...
                    for pred in preds:
                        context = user_model.dec_tok.decode(pred, skip_special_tokens=True)
                ...
                print(output_list)
                print('\n\n', '-' * 100, '\n\n')
        with open('3_baselines/result/predict_coda_conv_user.json', 'w', encoding='utf-8') as f:
