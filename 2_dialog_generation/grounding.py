...
def ground_TGConv(dialog):
    global nlp, matcher
    # print('load matcher')
    if nlp is None or matcher is None:
        nlp = spacy.load('en_core_web_sm', disable=['ner', 'parser', 'textcat'])
        # nlp.add_pipe(nl   p.create_pipe('sentencizer'))
        nlp.add_pipe('sentencizer')
        matcher = load_matcher(nlp, PATTERN_PATH)


    all_concepts = ground_mentioned_concepts(nlp, matcher, dialog)

    # print('hard ground q')
    if len(all_concepts) == 0:
        question_concepts = hard_ground(nlp, dialog, CPNET_VOCAB)  # not very possible

    # question_concepts = question_concepts -  answer_concepts
    all_concepts = sorted(list(all_concepts))
    return {"dialog": dialog, "ac": all_concepts}
...
def match_mentioned_concepts_TG(sents, num_processes):
    res = []
    with Pool(num_processes) as p:
        res = list(tqdm(p.imap(ground_TGConv, sents), total=len(sents)))
    return res
...
def prune_TG(data, cpnet_vocab_path):
    # reload cpnet_vocab
    with open(cpnet_vocab_path, "r", encoding="utf8") as fin:
        cpnet_vocab = [l.strip() for l in fin]
...
def ground_TG(statement_path, cpnet_vocab_path, pattern_path, output_path, num_processes=1, debug=False):
    global PATTERN_PATH, CPNET_VOCAB
    print('load cnpt vocab')
    if PATTERN_PATH is None:
        PATTERN_PATH = pattern_path
    ...
    for line in lines:
        if line == "":
            continue
        j = json.loads(line)
        for dialog in j["session"]:
            sents.append(dialog)

    print('match mention concept')
    res = match_mentioned_concepts_TG(sents, num_processes)
    print('prune')
    res = prune_TG(res, cpnet_vocab_path)
