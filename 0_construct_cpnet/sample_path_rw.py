    ...
    while num_sampled_nodes != max_len:
        ...
        while True:
            ...
            if not chosen_node in node_visited:
                ...
                rel_list = list(set([item for item in rel_list.keys()]))
                iteration_rel = 0
                while True:
                    index_of_rel = np.random.choice(len(rel_list), 1)[0]
                    chosen_rel = rel_list[index_of_rel]
                    if not chosen_rel in relation_visited:
                        flag_valid = True
                        break
                    else:
                        iteration_rel += 1
        ...
        relation_visited.add(chosen_rel)
        path.append(r2i[chosen_rel])
        path.append(e2i[chosen_node])
...
if __name__ == '__main__':
    ...
    output_path = os.path.join(output_dir, 'sample_paths.txt')
    ...
    with open(output_path, 'w') as fw:
        for curr_node in tqdm(not_visited_idx, desc='generating paths'):
            if not i2e[curr_node] in kg_simple:
                continue 
            for _id, _len in enumerate(path_lens):
                ...
                for pid in range(num_paths[_id]):
                    num_try = 0
                    while True:
                        path = random_walk(i2e[curr_node], kg_full, kg_simple, _len)
                        if len(path) > 0:
...
# nohup python -u sample_path_rw.py > sample_path_rw.log 2>&1
